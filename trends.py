...
def find_centroid(polygon):
    ...
    [1.0, 2.0, 0.0]
    """
    n = range(len(polygon) - 1)
    
    a = 0
    for i in n:
        a += ((longitude(polygon[i])*latitude(polygon[i+1])) - \
            (longitude(polygon[i+1])*latitude(polygon[i])))
    a *= (1/2)

    c_x = 0
    c_y = 0
    for i in n:
        num = (longitude(polygon[i])*latitude(polygon[i+1]) - \
            longitude(polygon[i+1])*latitude(polygon[i]))
        c_x += ((longitude(polygon[i]) + longitude(polygon[i+1])) * num)
        c_y += ((latitude(polygon[i]) + latitude(polygon[i+1])) * num)

    if a != 0:
        c_x *= 1/(6*a)
        c_y *= 1/(6*a)
    else:
        c_x = longitude(polygon[0])
        c_y = latitude(polygon[0])

    return [c_y, c_x, abs(a)]
...
def find_state_center(polygons):
    ...
    for polygon in polygons:
        a = find_centroid(polygon)[2]
        lat += find_centroid(polygon)[0] * a
        lon += find_centroid(polygon)[1] * a
        area += a
...
def group_tweets_by_state(tweets):
    ...
    def find_state(n): # Takes in position and returns closest state
        keys = get_keys(us_states)
        closest = keys[0]
        
        for key in keys:
            current = find_state_center(get_value_from_key(us_states, closest))
            compare = find_state_center(get_value_from_key(us_states, key))
            if geo_distance(current, n) > geo_distance(compare, n):
                closest = key

        return closest

    for tweet in tweets:
        state = find_state(tweet_location(tweet))
        
        if state in get_keys(tweets_by_state):
            add = get_value_from_key(tweets_by_state, state)
            tweets_by_state = add_value(tweets_by_state, state, add+[tweet])
        else:
...
def average_sentiments(tweets_by_state):
    ...
    keys= get_keys(tweets_by_state)

    for i in range(len(keys)):
        sentiment, value = [], get_value_from_key(tweets_by_state, keys[i])
        ...
        if sentiment != []: #not empty
            avg = 0
            for j in sentiment:
                avg += sentiment_value(j)
            avg /= len(sentiment)
